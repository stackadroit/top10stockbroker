...
class GoldSilverChart extends React.Component {
  constructor(props) {
    super(props);
    var dur=$('.gold-silver-graph').find('li.active a').data("filter");
    this.state = {
    ...
    componentDidMount(){
  	...
  	getData(){
      	var dur=$('.gold-silver-graph').find('li.active a').data("filter");
      	var selli=$('.gold-silver-graph').find('li.active a').data("element");
     	  var resp_div=$('.gold-silver-graph').find('li.active a').data("chart-element");
      	const data = new FormData();
        var post_id=$('.gold-silver-graph').find('li.active a').data("post-id");
      	data.append('dur', dur);
        data.append('postId', post_id);
      	data.append('action', 'get_gold_silver_price_graph_data_');
      	data.append('nonce', global_vars.ajax_nonce);

      	axios.post(global_vars.ajax_url, data)
        .then(response => {

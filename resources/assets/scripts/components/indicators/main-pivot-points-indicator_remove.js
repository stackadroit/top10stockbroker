...
import { Select, MenuItem, Button,InputLabel,FormControl} from "@material-ui/core"; 
import { useState,setState } from "react";
import CustomToolbar from "./custom-toolbar";

class MainPivotPointsIndicator extends React.Component {
    constructor(props){
        super(props);
        this.state = {
            error : null,
      ...
      const columns = [
        {
            ...
            options: {
              ...
              customBodyRender: (value, tableMeta, updateValue) => {
                return (
                  <a href={tableMeta.rowData[0]}>{value}</a>
                );
              }
            }
        },
    ...
    getData(){
      ...
      //   data.append('paged',1);
      //   axios.post(global_vars.apiServerUrl + '/apiblock/main-pivot-points-indicator-list', data)
      //     .then(res => {
    ...
    render() {
        const {error,columns,selectedFilter, isLoaded, lists,filter_list} = this.state;
        const options = {
            selectableRows: 'none', 
            hasIndex: true, 
            search:false,
        ...
        if(error){
           ...
           return (
             <ContentLoader viewBox="0 0 400 60" height={60} width={1000}>
...
export default MainPivotPointsIndicator;
